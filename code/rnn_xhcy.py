...
class rnn_xhcy():
    ...
    def __init__(self, dim_i, dim_h, dim_c, dim_o, learning_rate, factor_inicializacion=0.01, 
        ...
        self.Whh = tf.Variable(np.random.randn(dim_h, dim_h)*factor_inicializacion, 
                               dtype=tf.float64, 
                               name = 'Whh')
        self.Whc = tf.Variable(np.random.randn(dim_c, dim_h)*factor_inicializacion, 
                               dtype=tf.float64, 
                               name = 'Why')
        self.Wxc = tf.Variable(np.random.randn(dim_c, dim_i)*factor_inicializacion,
                               dtype=tf.float64,
                               name = 'Wxc')
        self.Wcy = tf.Variable(np.random.randn(dim_o, dim_c)*factor_inicializacion,
                               dtype=tf.float64,
                               name = 'Wcy')
        ...
        self.by = tf.Variable(np.random.randn(dim_o, 1)*factor_inicializacion,
                              dtype=tf.float64,
                              name = 'by')
        self.bc = tf.Variable(np.random.randn(dim_c, 1)*factor_inicializacion,
                              dtype=tf.float64,
                              name = 'bc')
        ...
        self.l1reg_mem = tf.placeholder(dtype=tf.float64,
                                        shape=[1],
                                        name = 'L1_reg_MEM')
        
        self.l1reg_cpu = tf.placeholder(dtype=tf.float64,
                                        shape=[1],
                                        name = 'L1_reg_CPU')
        ...
        for i in range(seq_len):
            ...
            self.cz.append(tf.matmul(self.Wxc, self.x[:, i, :]) + 
                           self.bc + 
                           #self.noise / 10.0 * tf.random_normal([dim_c, num_batches], dtype=tf.float64) +
                           tf.matmul(self.Whc, self.h[-1]))
            self.c.append(tf.tanh(self.cz[-1], name='c'))
            
            self.yz.append(tf.matmul(self.Wcy, self.c[-1]) + self.by)
        ...
        self.cz_one_step = tf.matmul(self.Wxc, self.x_one_step) + self.bc + tf.matmul(self.Whc, self.h_one_step)
        self.c_one_step = tf.tanh(self.cz_one_step)
        
        self.yz_one_step = tf.matmul(self.Wcy, self.c_one_step) + self.by
        self.y_one_step = tf.nn.softmax(tf.transpose(self.yz_one_step))

        # Regularizacion L1
        regL1Wxh = tf.reduce_sum(tf.abs(self.Wxh), name='regL1_Wxh') * self.l1reg_mem
        regL1Whh = tf.reduce_sum(tf.abs(self.Whh), name='regL1_Whh') * self.l1reg_mem
        regL1Whc = tf.reduce_sum(tf.abs(self.Whc), name='regL1_Whc') * self.l1reg_mem
        regL1Wxc = tf.reduce_sum(tf.abs(self.Wxc), name='regL1_Wxc') * self.l1reg_cpu
        regL1Wcy = tf.reduce_sum(tf.abs(self.Wcy), name='regL1_Wcy') * self.l1reg_cpu
        
        #regL1bh = tf.reduce_sum(tf.abs(self.bh), name='regL1_bh') * self.l1reg
        #regL1by = tf.reduce_sum(tf.abs(self.by), name='regL1_by') * self.l1reg
        #regL1bc = tf.reduce_sum(tf.abs(self.bc), name='regL1_bc') * self.l1reg

        self.regTotal = regL1Wxh + regL1Whh + regL1Whc + regL1Wxc + regL1Wcy
        ...
        self.shock = [self.Wxh.assign(self.Wxh + tf.random_normal([dim_h, dim_i], dtype=tf.float64) * shock),
                      ...
                      self.Wxc.assign(self.Wxc + tf.random_normal([dim_c, dim_i], dtype=tf.float64) * shock)]
    ...
    def __estados_run__(self, longitud, data_input):
        ...
        for i in range(0, longitud, self.seq_len):
            ...
            feed_dict[self.h_prev] = h_prev
            feed_dict[self.noise] = [0.0]
            y, h, hz, c = self.sess.run((self.y, self.h, self.hz, self.c), feed_dict=feed_dict)
            h_prev = h[-1]
            hs.append(np.array(h)[:, :, 0])
            hzs.append(np.array(hz)[:, :, 0])
            ys.append(np.array(y)[:, 0, :])
            cs.append(np.array(c)[:, :, 0])
        ...
        return hs.reshape(hs.shape[0]*hs.shape[1], hs.shape[2]), ys.reshape(ys.shape[0]*ys.shape[1], ys.shape[2]), hzs.reshape(hs.shape[0]*hs.shape[1], hs.shape[2]), cs.reshape(cs.shape[0]*cs.shape[1], cs.shape[2])
    ...
    def estados(self, longitud=1000, input_file='', input_files=[], output_file='', inputs='', outputs='', desplazar=True, 
        ...
        hs, ys, hzs, cs = self.__estados_run__(np.min([longitud, dataset_len]), data_input)
        
        return hs, ys, input_final, output_final, data_dic, transform_dic, hzs, cs
    ...
    def sample_prediction(self, args_prediction, sample_len=100, reset_internal_state=True, add_diversity=True, temperature=1.0, next_char_ini='$'):
